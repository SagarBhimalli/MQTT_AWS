...
/* Smartap topics */
#define TOPIC_SMART_TOUCH "/voni_home" /* topic prefix */
const int TOPIC_LEN = strlen(TOPIC_SMART_TOUCH);
...
/* AWS MQTT host URL */
static char mqttHostUrl[255] = "a2s1s3gc5un2ut-ats.iot.ap-south-1.amazonaws.com"; //"a3gjsr00gjuzw5-ats.iot.us-west-1.amazonaws.com";
...
static void mqttCommunicationTask(void *pArg)
{
    ...
    while (1)
    {
        ...
        if (doPublishStatus == true)
        {
            ...
            publishSwitchStatus(); // Commented By Sagar
            publishSensorStatus();
        }
...
static void mqttReceiveCb(AWS_IoT_Client *pClient, char *topicName, uint16_t topicNameLen, IoT_Publish_Message_Params *params, void *pData)
{
    ...
    ESP_LOGI(TAG, "Payload_Received %.*s\t%.*s", topicNameLen, topicName, (int)params->payloadLen, (char *)params->payload);
    ...
    // if (strncmp(topicName, Topic_Factory_Reset, strlen(Topic_Factory_Reset)) == 0)
    // {
    //     DoFactoryReset();
    // }
    // if (strncmp(topicName, Topic_Dev_Restart, strlen(Topic_Dev_Restart)) == 0)
    // {
    //     DoSystemRestart();
    // }
